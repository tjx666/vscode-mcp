...
export function registerGetDiagnostics(server: McpServer) {
  ...
  }, async ({ workspace_path, __NOT_RECOMMEND__filePaths, sources, severities }) => {
    const dispatcher = await createDispatcher(workspace_path);

    try {
      const result = await dispatcher.dispatch("getDiagnostics", { __NOT_RECOMMEND__filePaths, sources, severities });
      ...
      if (result.files.length === 0) {
        return {
          content: [{
            type: "text",
            text: __NOT_RECOMMEND__filePaths.length === 0
      ...
      if (filesWithDiagnostics.length === 0) {
        summary = __NOT_RECOMMEND__filePaths.length === 0
        ...
        const header = __NOT_RECOMMEND__filePaths.length === 0

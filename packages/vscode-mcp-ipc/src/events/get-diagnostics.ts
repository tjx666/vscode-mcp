...
export const GetDiagnosticsInputSchema = z.object({
  ...
  severities: z.array(z.enum(['error', 'warning', 'info', 'hint'])).optional().default(['error', 'warning', 'info']).describe('Array of severity levels to include. Supported values: "error", "warning", "info", "hint". Defaults to ["error", "warning", "info"] (excludes "hint" which is usually editor suggestions rather than issues).'),
}).strict();

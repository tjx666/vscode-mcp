...
export const getDiagnosticsTool: ToolDefinition<typeof GetDiagnosticsInputSchema> = {
  name: 'get_diagnostics',
  cliName: 'get-diagnostics',
  title: 'Get Diagnostics',
  description: DESCRIPTION,
  schema: GetDiagnosticsInputSchema,
  ...
  async handler(params, { workspacePath }) {
    ...
    if (result.files.length === 0) {
      return __NOT_RECOMMEND__filePaths.length === 0
    ...
    const output = filesWithDiagnostics
      .map((file) => {
        const diagnosticsList = file.diagnostics
          .map((diag) => {

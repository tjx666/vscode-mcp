...
import { activateTerminalLauncher } from './terminal-launcher';
...
export async function activate(context: vscode.ExtensionContext) {
    ...
    // Terminal launcher is independent of workspace / socket server, so it keeps
    // working even when no workspace is open or socket start fails.
    activateTerminalLauncher(context);
    ...
    try {
        ...
        const sleepCommandDisposable = vscode.commands.registerCommand(
            'vscode-mcp-bridge.sleep',
        ...
        const copyOpenedFilesPathDisposable = vscode.commands.registerCommand(
            'vscode-mcp-bridge.copyOpenedFilesPath',
            async (options?: CopyOpenedFilesPathOptions) => {
                await copyOpenedFilesPathCommand(options);
            },
        );
        context.subscriptions.push(copyOpenedFilesPathDisposable);

        const copyCurrentSelectionReferenceDisposable = vscode.commands.registerCommand(
            'vscode-mcp-bridge.copyCurrentSelectionReference',
            async (options?: CopyCurrentSelectionReferenceOptions) => {
        ...
        vscode.window.showErrorMessage(
            `VSCode MCP Bridge: Failed to start socket server - ${error}`,
        );
    }
}
